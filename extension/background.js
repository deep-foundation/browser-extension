...
chrome.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
  chrome.storage.sync.get(["CONTAINER_LINK_ID", "PACKAGE_NAME", "GQL_URL", "GQL_TOKEN"], async (data) => {
    console.log({data});
    if (data.CONTAINER_LINK_ID && data.PACKAGE_NAME && data.GQL_URL && data.GQL_TOKEN) {
      if (message.action === "uploadHistory") {
        // Code to execute when UPLOAD HISTORY button is clicked
        chrome.history.search({ text: "", maxResults: 2 }, async (history) => {
          await executeUploadHistory(history, data.GQL_URL, data.GQL_TOKEN, data.PACKAGE_NAME, data.CONTAINER_LINK_ID);
        });
      } else if (message.action === "uploadTabs") {
        // Code to execute when SUBSCRIBE TABS button is clicked
        chrome.tabs.query({}, async (tabs) => {
          await executeInsertTabs(tabs, data.GQL_URL, data.GQL_TOKEN, data.PACKAGE_NAME, data.CONTAINER_LINK_ID);
        });
        chrome.tabs.onActivated.addListener(async (activeInfo) => {
          // Add code to execute when a tab is activated
        ...
        chrome.tabs.onCreated.addListener(async (tab) => {
          // Add code to execute when a new tab is created
          await executeInsertTabs([tab], data.GQL_URL, data.GQL_TOKEN, data.PACKAGE_NAME, data.CONTAINER_LINK_ID);
          console.log("Tab created:", tab.id);
        });
